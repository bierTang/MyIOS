...
@objc public class LTPageTitleView: UIView {
    ...
    var upDateMake:Bool = false{
        didSet{
            sliderScrollView.frame = self.frame
//            sliderLineView.frame =  CGRect(x: layout.lrMargin, y: bounds.height - layout.bottomLineHeight - layout.pageBottomLineHeight, width: 0, height: layout.bottomLineHeight)
            pageBottomLineView.frame = CGRect(x: 0, y: bounds.height - layout.pageBottomLineHeight, width: bounds.width, height: layout.pageBottomLineHeight)
//            setupButtonsLayout()
        }
    }
...
//        这里因为titleview要比pageview宽度要窄，所以pageview直接要取屏幕宽度，隐藏这个
//        let totalW = bounds.width
        let totalW = Screen_Width
        isClick = true
...
//        let scrollW = bounds.width
        let scrollW = Screen_Width
        
      //        这里因为titleview要比pageview宽度要窄，所以pageview直接要取屏幕宽度，隐藏这个
//        let scrollContenSizeW: CGFloat = bounds.width * CGFloat(titles.count)
        let scrollContenSizeW: CGFloat = Screen_Width * CGFloat(titles.count)
