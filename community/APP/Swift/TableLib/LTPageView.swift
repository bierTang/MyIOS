...
public class LTPageView: UIView {
    ...
    /* 更新titleviewl宽度  */
       @objc public var titleWidth = Screen_Width {
           didSet {
            pageTitleView.frame = CGRect(x: 0, y: 0, width: titleWidth, height: self.layout.sliderHeight)
            self.pageTitleView.upDateMake = true
           }
       }
...
extension LTPageView {
    private func setupTitleView() -> LTPageTitleView {
        let pageTitleView = LTPageTitleView(frame: CGRect(x: 0, y: 0, width: self.layout.allSliderWidth, height: self.layout.sliderHeight), titles: titles, layout: layout)
...
extension LTPageView {
    ...
    public func glt_createViewController(_ index: Int)  {
        ...
        VC.didMove(toParent: currentViewController)
