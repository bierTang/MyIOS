...
-(void)initUI{
    ...
    for (int i = 0; i<10; i++) {
        UIImageView *imageview = [[UIImageView alloc]initWithFrame:CGRectMake(35*K_SCALE+66*K_SCALE*(i%5), 16*K_SCALE+76*K_SCALE*(i/5), 40*K_SCALE, 40*K_SCALE)];
...
-(void)refreshCell:(NSArray *)arr memBerNum:(NSInteger)num{
    ...
    for (int i=0; i<max; i++) {
        ...
        [self.ImgArr[i] sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",mainHost,model.avatar]] placeholderImage:[UIImage imageNamed:@"headImg_base_3"]];
