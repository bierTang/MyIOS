...
#define kTabDefautFontSize 13.0
...
CGFloat itemInteval = 25;
@interface CustomSlideMenu () <UIScrollViewDelegate>
//bg
...
@property (nonatomic,strong)NSString *unselectColor;
@property (nonatomic,strong)NSString *selctColorString;
//data
...
@property (nonatomic, strong) NSMutableArray *downSignArray;
...
@property(assign)NSUInteger numOfItem;//一屏幕 显示 个数
@property(nonatomic,strong)CustomTabItem *popTitleItem;
...
- (void)layoutSubviews {
    if(_isNeedRefreshLayout) {
        ...
        _tabItemWidth = _tabSize.width/_numOfItem ;
        NSInteger num ;
        if (_numberOfTabItems < _numOfItem) {
           num = _numOfItem;
        }else{
            num = _numberOfTabItems;
        }
        self.scrollView.frame = CGRectMake(0, 0, _tabSize.width, _tabSize.height);
        self.scrollView.contentSize = CGSizeMake(_tabItemWidth*num, 0);
        for(NSInteger i = 0; i < _tabItems.count; i++) {
            CustomTabItem *tabItem = (CustomTabItem *)_tabItems[i];
            tabItem.font = i==_selectedTabIndex?_tabItemSelectFont:_tabItemFont;
            tabItem.frame = CGRectMake([_itemOriginXArray[i] floatValue],0,[_itemWidthArray[i] floatValue], _tabSize.height);
            if (self.isShowDown) {
                UIImageView *downSign = _downSignArray[i];
                [downSign makeConstraints:^(MASConstraintMaker *make) {
                    make.left.equalTo(tabItem.right).offset(0);
                    make.centerY.equalTo(tabItem.centerY);
        ...
        for(NSInteger i = 0; i < _numberOfTabItems; i++) {
            UIView *childView = _childControllers[i];
            childView.frame = CGRectMake(WIDTH(self)*i,0, WIDTH(self), HEIGHT(self)-_tabSize.height-70);
        }
...
- (void)initBaseSettings {
    ...
    _tabItemFont = [UIFont boldSystemFontOfSize:kTabDefautFontSize];
    _maxNumberOfPageItems = kMaxNumberOfPageItems;
    _tabItems = [NSMutableArray array];
    _downSignArray = [NSMutableArray array];
    _bodyBackgroundColor = [UIColor whiteColor];
    self.unselectColor = @"666666";
    
    _unSelectedColor = [UIColor colorWithHexString: self.unselectColor];
    _selectedColor = [UIColor colorWithHexString:@"ffffff"];
    _isNeedRefreshLayout = YES;
    _isChangeByClick = NO;
    _bodyBounces = NO;
    _titleStyle = CustomPageTabTitleStyleDefault;
    _indicatorStyle = CustomPageTabIndicatorStyleDefault;
    _minScale = kMinScale;
    _selectedColorR = 1;
    _selectedColorG = 0;
    _selectedColorB = 0;
...
- (void)initScrollView {
    ...
    for(NSInteger i = 0; i < _numberOfTabItems; i++) {
        CustomTabItem *tabItem = [[CustomTabItem alloc] init];
        ...
        if (self.isShowDown) {
            UIImageView *downSign = [[UIImageView alloc] init];
            downSign.hidden = i==_selectedTabIndex?NO:YES;
            downSign.image = [UIImage imageNamed:@"New_downsign"];
            [_downSignArray addObject:downSign];
            [self.scrollView addSubview:downSign];
        }
    }
}
...
- (void)initItemWidthArray{
    ...
    for (int i = 0; i< self.childTitles.count; i++) {
        NSString *item = self.childTitles[i];
...
- (void)initMainView {
    ...
    for(NSInteger i = 0; i < _numberOfTabItems; i++) {
        UIView *childView = _childControllers[i];
        [self.bodyView addSubview:childView];
    }
}
...
- (void)reviseTabContentOffsetBySelectedIndex:(BOOL)isAnimate {
    CustomTabItem *currentTabItem = _tabItems[_selectedTabIndex];
    CGFloat selectedItemCenterX = currentTabItem.center.x;
    
    CGFloat reviseX;
    if(selectedItemCenterX + _tabSize.width/2.0 >= self.scrollView.contentSize.width) {
        reviseX = self.scrollView.contentSize.width - _tabSize.width; //不足以到中心，靠右
    ...
    if(fabs(self.scrollView.contentOffset.x - reviseX)<1 || !isAnimate) {
        [self finishReviseTabContentOffset];
    }
    [self.scrollView setContentOffset:CGPointMake(reviseX, 0) animated:isAnimate];
}
...
- (void)finishReviseTabContentOffset {
    ...
    _isNeedRefreshLayout = YES;
    _isChangeByClick = NO;
    if([self.customDelegate respondsToSelector:@selector(pageTabViewDidEndChange:)]) {
        if(_lastSelectedTabIndex != _selectedTabIndex) {
            [self.customDelegate pageTabViewDidEndChange:_selectedTabIndex];
        }
    }
...
- (void)changeSelectedItemToNextItem:(NSInteger)nextIndex {
    CustomTabItem *currentTabItem = _tabItems[_selectedTabIndex];
    CustomTabItem *nextTabItem = _tabItems[nextIndex];
    self.popTitleItem = nextTabItem;
    if (self.isShowDown) {
        UIImageView *currentdownSign = _downSignArray[_selectedTabIndex];
        UIImageView *nextDownSign = _downSignArray[nextIndex];
        currentdownSign.hidden = YES;
        nextDownSign.hidden = NO;
    }
    currentTabItem.textColor = _unSelectedColor;
    nextTabItem.textColor = _selectedColor;
    currentTabItem.font = _tabItemFont;
    nextTabItem.font = _tabItemSelectFont;
...
- (void)resetTabItemScale {
    for(NSInteger i = 0; i < _numberOfTabItems; i++) {
        CustomTabItem *tabItem = _tabItems[i];
...
- (void)addIndicatorViewWithStyle {
    switch (_indicatorStyle) {
        case CustomPageTabIndicatorStyleDefault:
        case CustomPageTabIndicatorStyleFollowText:
        case CustomPageTabIndicatorStyleStretch:
...
- (void)layoutIndicatorView {
    ...
    CustomTabItem *selecedTabItem = _tabItems[_selectedTabIndex];
    self.indicatorView.frame = CGRectMake(selecedTabItem.center.x-indicatorWidth/2.0-_scrollView.contentOffset.x, _tabSize.height-_indicatorHeight, indicatorWidth, _indicatorHeight);
}
...
- (void)changeChildControllerOnClick:(UITapGestureRecognizer *)tap {
    ...
    if(nextIndex != _selectedTabIndex) {
        if(_titleStyle == CustomPageTabTitleStyleDefault) {
            [self changeSelectedItemToNextItem:nextIndex];
        }
        
        _isChangeByClick = YES;
        _scrollView.userInteractionEnabled = NO; //防止快速切换
        ...
        if (self.showPopViewBlock) {
            self.showPopViewBlock(tap);
        }
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if(scrollView == self.scrollView) {
        ...
        //调整title
        switch (_titleStyle) {
            case CustomPageTabTitleStyleDefault:
                [self changeTitleWithDefault];
                break;
            case CustomPageTabTitleStyleGradient:
                [self changeTitleWithGradient];
                break;
            case CustomPageTabTitleStyleBlend:
...
- (void)changeTitleWithGradient {
    if(_leftItemIndex != _rightItemIndex) {
        ...
        CustomTabItem *leftTabItem = _tabItems[_leftItemIndex];
        CustomTabItem *rightTabItem = _tabItems[_rightItemIndex];
...
- (void)changeIndicatorFrame {
    ...
    self.indicatorView.frame = CGRectMake(nowIndicatorCenterX-nowIndicatorWidth/2.0-_scrollView.contentOffset.x, ORIGIN_Y(self.indicatorView), nowIndicatorWidth, HEIGHT(self.indicatorView));
}
...
- (void)changeIndicatorFrameByStretch {
    ...
    if(relativeLocation <= 0.5) {
        ...
        nowFrame.origin.x = (leftTabItem.center.x-self.scrollView.contentOffset.x)-_indicatorWidth/2.0;
    } else {
        nowFrame.size.width = _indicatorWidth+_tabItemWidth*((1-relativeLocation)/0.5);
        nowFrame.origin.x = (rightTabItem.center.x-self.scrollView.contentOffset.x)+_indicatorWidth/2.0-nowFrame.size.width;
    }
...
- (CGFloat)getIndicatorWidthWithTitle:(NSString *)title {
    if(_indicatorStyle == CustomPageTabIndicatorStyleDefault || _indicatorStyle == CustomPageTabIndicatorStyleStretch) {
...
-(void)setCurrentSelectTitle:(NSString *)currentSelectTitle{
    _currentSelectTitle = currentSelectTitle;
    self.popTitleItem.text  = _currentSelectTitle;
}
...
-(void)setIndicatorViewColor:(UIColor *)indicatorViewColor{
    ...
    self.indicatorView.backgroundColor = _indicatorViewColor;
}

- (void)setTabBackgroundColor:(UIColor *)tabBackgroundColor {
    _tabBackgroundColor = tabBackgroundColor;
    self.scrollView.backgroundColor = _tabBackgroundColor;
}
...
- (void)setSelectedTabIndex:(NSInteger)selectedTabIndex {
    ...
    if(selectedTabIndex >= 0 && selectedTabIndex < _numberOfTabItems && _selectedTabIndex != selectedTabIndex) {
        ...
        if(_titleStyle == CustomPageTabTitleStyleGradient) {
            [self resetTabItemScale];
        }
        if([self.customDelegate respondsToSelector:@selector(pageTabViewDidEndChange:)]) {
            [self.customDelegate pageTabViewDidEndChange:selectedTabIndex];
        }
    }
}
...
- (void)setSelectedColor:(UIColor *)selectedColor {
    ...
    CustomTabItem *tabItem = _tabItems[_selectedTabIndex];
    tabItem.textColor = _selectedColor;
    self.indicatorView.backgroundColor = [UIColor colorWithHexString:@"235311"];
...
-(void)setTabItemSelectFont:(UIFont *)tabItemSelectFont{
    _tabItemSelectFont = tabItemSelectFont;
}

- (void)setTitleStyle:(CustomPageTabTitleStyle)titleStyle {
    if(_titleStyle == CustomPageTabTitleStyleDefault) {
...
- (void)setIndicatorStyle:(CustomPageTabIndicatorStyle)indicatorStyle {
    if(_indicatorStyle == CustomPageTabIndicatorStyleDefault) {
