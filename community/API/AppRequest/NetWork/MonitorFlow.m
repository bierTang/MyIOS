...
static MonitorFlow * sharedInstance = nil;
...
- (MonitorData *)getMonitorDataDetail{
    ...
    NSString *dataName;success = getifaddrs(&addrs) == 0;
    if (success){cursor = addrs;
        while (cursor != NULL){dataName = [NSString stringWithFormat:@"%s",cursor->ifa_name];
...
//            float wifiReceived = monitor.wifiReceived - self.tempWifiReceived;
            ...
            self.netSendSpeed = [NSString stringWithFormat:@"%.0f KB/s",wifiSend];
    }
        break;
...
//            float wwanReceived = monitor.wifiReceived - self.tempWWANSend;
            self.netSendSpeed = [NSString stringWithFormat:@"%.0f KB/s",wwanSend];
...
- (void)startMonitor {
    [self currentFlow];
    self.timer = [NSTimer timerWithTimeInterval:1.0 target:self selector:@selector(refreshFlow) userInfo:nil repeats:YES];[[NSRunLoop currentRunLoop] addTimer:self.timer forMode:NSRunLoopCommonModes];
